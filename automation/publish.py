...
BASE='https://home-project-wise.github.io/home-project-wise'; GOAT='<script data-goatcounter="https://homeprojectwise.goatcounter.com/count" async src="//gc.zgo.at/count.v5.js" crossorigin="anonymous" integrity="sha384-atnOLvQb9t+jTSipvd75X2yginT4PjVbqDdlJAmxMm+wYelFmeR6EmLP5bYeoRVQ"></script>'
def image_key(url):
...
def render_article(d):
 ...
 images=d.get('images') or [{'url':d['image'],'alt':d.get('image_alt','Home improvement project'),'caption':''}]
 hero=images[0]
 gallery=''.join('<figure><img class="article-image" src="'+escape(i['url'],quote=True)+'" alt="'+escape(i.get('alt','Home improvement project'),quote=True)+'" loading="lazy"><figcaption>'+escape(i.get('caption',''),quote=False)+'</figcaption></figure>' for i in images[1:])
 faq=[]
 for m in re.finditer(r'<h3>(.*?)</h3>\s*<p>(.*?)</p>',d['body_html'],re.S|re.I): faq.append({'@type':'Question','name':re.sub('<[^>]+>','',m.group(1)).strip(),'acceptedAnswer':{'@type':'Answer','text':re.sub('<[^>]+>','',m.group(2)).strip()}})
 schema=json.dumps({'@context':'https://schema.org','@type':'BlogPosting','headline':d['title'],'description':d['description'],'mainEntityOfPage':url,'author':{'@type':'Organization','name':'HomeProjectWise Editorial Team'},'publisher':{'@type':'Organization','name':'HomeProjectWise'}},ensure_ascii=False)
 faq_schema=json.dumps({'@context':'https://schema.org','@type':'FAQPage','mainEntity':faq},ensure_ascii=False)
...
def rebuild_home(rows):
 ...
 pattern=r'<section id="guides" class="section">.*?</section>'; replacement='<section id="guides" class="section"><div class="section-head"><div><p class="eyebrow">THE LATEST</p><h2>Things worth knowing before you start.</h2></div><a href="guides.html">All guides →</a></div><div class="cards">'+cards+'</div></section>'
...
def main():
 items=[]; registry=load_registry(); used=set(registry.get('used_ids',[]))
 ...
 for d in due:
  ...
  if (ARTICLES/(d['slug']+'.html')).exists(): raise RuntimeError('Refusing duplicate slug: '+d['slug'])
  render_article(d); used.add(key); registry.setdefault('used_ids',[]).append(key); registry.setdefault('used_urls',[]).append(d['image'].split('?')[0]); d['_path'].unlink(); published.append(d['slug'])
