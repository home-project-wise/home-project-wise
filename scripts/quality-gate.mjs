...
const allHtml = [fs.readFileSync(path.join(root, 'index.html'),'utf8'), ...files.map(f => fs.readFileSync(path.join(articlesDir,f),'utf8'))];
let errors = [];
...
for (const file of files) {
  ...
  if (!title || title.length < 25 || title.length > 75) errors.push(`${file}: title length is outside the editorial range`);
...
for (const html of allHtml) {
  for (const m of html.matchAll(/<img[^>]+src=["']([^"']+)["']/gi)) {
    if (seenImages.has(m[1])) errors.push(`duplicate image URL: ${m[1]}`);
    seenImages.add(m[1]);
  }
}

for (const html of allHtml) {
  const bad = [...html.matchAll(/href=["']([^"'#][^"']*)["']/gi)].map(m=>m[1]).filter(u=>u.endsWith('.html'));
  for (const u of bad) {
    const target = path.join(root, u.replace(/^\.\//,''));
    if (!fs.existsSync(target)) errors.push(`broken internal link: ${u}`);
  }
}

if (errors.length) { console.error(errors.join('\n')); process.exit(1); }
console.log(`Quality gate passed: ${files.length} article(s), ${seenImages.size} unique image URL(s).`);
